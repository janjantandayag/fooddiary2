...
Class Database {
	...
	public function getEntriesDate($start,$end){
		$uid = $_SESSION['userId'];
		$stmt = $this->conn->prepare("SELECT * FROM entries,item WHERE entries.user_id = $uid AND entries.entry_id = item.entry_id AND (DATE_FORMAT(entry_date,'%Y-%m-%d')  >= '$start' AND DATE_FORMAT(entry_date,'%Y-%m-%d')  <= '$end')"); 
 		$stmt->execute(); 
 		$result = $stmt->fetchAll();
 		return $result;
	}
